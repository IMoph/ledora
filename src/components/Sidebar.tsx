...
const Sidebar = ({
  ...
  if (isMobile) {
    return <div className="fixed bottom-4 right-4 z-50">
        ...
        {!collapsed && <div className="fixed bottom-24 right-4 flex flex-col gap-2 animate-fade-in">
            <Button onClick={onPanelsClick} variant="outline" className="bg-white shadow-lg rounded-full flex gap-2 items-center">
              <LayoutPanelLeft size={18} />
              Painéis
            </Button>
            <Button onClick={onTutorialClick} variant="outline" className="bg-white shadow-lg rounded-full flex gap-2 items-center">
              <Info size={18} />
              Tutorial
            </Button>
          </div>}
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-2 flex-1">
          <div className="space-y-2">
            <Button variant="ghost" className={`w-full justify-${collapsed ? 'center' : 'start'}`}>
              <Calculator className="mr-2" />
              <span className={textDisplay}>Calculadora</span>
            </Button>
            <Button variant="ghost" className={`w-full justify-${collapsed ? 'center' : 'start'}`} onClick={onPanelsClick}>
              <LayoutPanelLeft className="mr-2" />
              <span className={textDisplay}>Painéis</span>
            </Button>
            <Button variant="ghost" className={`w-full justify-${collapsed ? 'center' : 'start'}`} onClick={onTutorialClick}>
              <Info className="mr-2" />
